...
pub fn draw_triangles(sr: &SystemResources, s: &SceneState) -> Result<(), Error> {
    ...
    {
        let mut position_storage = s.resource_storage(Position::id()).unwrap().write().unwrap();
        let position_storage = (&mut **position_storage as &mut dyn Any)
            .downcast_mut::<IdMappedResourceStorage<EntityId, Position>>()
            .unwrap();
